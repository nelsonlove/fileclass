...
| **Global fileClass** | A baseline carried by **every** note, on top of whatever classes it names itself; the note's own class wins any key both declare. Never applied to a `.fileclass` definition, which declares classes rather than uses them. Accepts a wikilink (`[[Default.fileclass]]`, the form used everywhere else), a bare name (`Default`), a full name (`Default.fileclass`), or a path; a value matching no class is ignored silently. Leave empty to disable. |
