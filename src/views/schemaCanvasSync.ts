...
export function schemaCanvasPath(plugin: FileclassPlugin): string {
	...
	// No class folder in this fork (definitions live anywhere), so the vault root is
	// the only place that is right for every vault.
	return normalizePath("Schema.canvas");
}
