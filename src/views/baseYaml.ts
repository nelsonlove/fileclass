 ...
 * `list(fileClass).contains("Book.fileclass")`.
 *
 * **Wikilink-references fork.** Upstream emits `alias.containsAny("Name")`, which
 * solves the cardinality half of the problem: a note may carry **several** classes,
 * and then the property is a YAML list, which no equality test matches.
 *
 * This fork's values are **Links**, not strings, so the clause also has to survive
 * a link-typed property. `list(...)` normalizes a scalar-or-list link property to a
 * list and `.contains(...)` matches the linked definition by name — the idiom real
 * vault bases use for link-valued properties, and the one this fork has shipped
 * against since 0.2.0. It covers the multi-class case too, so it replaces rather
 * than wraps upstream's clause.
 */
...
/**
 * The forms the clause above used to take, still present in bases generated by an
 * earlier version — upstream's `alias.containsAny("Name")` and, before it, plain
 * `alias == "Name"`.
 *
 * Kept so `isGeneratedScopeFilter` recognises those filters as ours: a sync then
 * repairs them in place. Treating them as hand-written would be the worse failure —
 * a filter that matches no link-valued property at all would be preserved out of
 * politeness, leaving the view permanently empty.
 */
function legacyFileClassFilterClauses(alias: string, fileClassName: string): string[] {
	const name = JSON.stringify(fileClassName);
	return [`${alias}.containsAny(${name})`, `${alias} == ${name}`];
}
...
export function isGeneratedScopeFilter(filters: unknown, scope: ClassScope): boolean {
	...
	if (typeof only === "string") {
		return (
			...
			legacyFileClassFilterClauses(scope.alias, scope.name).includes(only)
		);
	}
	...
	const generated = new RegExp(
		`^(?:list\\(${alias}\\)\\.contains\\(|${alias}\\.containsAny\\(|${alias} == |file\\.inFolder\\(|file\\.hasTag\\()`
	);
