...
export class FileClassOptionsModal extends Modal {
	...
	private classFolderReason(folder: string): string | null {
		// Upstream keeps class notes in one folder and refuses to bind it. This fork
		// discovers `.fileclass` definitions vault-wide, so there is no such folder —
		// and a definition can never be a note of a class anyway, since binding reads
		// only markdown notes. Nothing to refuse; every folder stays selectable.
		void folder;
		return null;
	}
