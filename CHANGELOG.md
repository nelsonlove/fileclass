...
### Merged from upstream (0.2.8 → 0.2.12)

- Reverse relations: **Insert notes that point here** builds a Base view of the notes
  whose link field points at the current note (#154), plus a schema canvas (#149),
  bookmark-group binding that actually walks the bookmark tree, nested-tag binding,
  bulk insert-missing across a class, **Reorder properties**, and an icon for the
  `fileclass-table` view type.
- The Global fileClass setting is a dropdown, and never applies to a class definition.

### Fixed

- **A `.fileclass` definition is never bound as a note of a class.** Binding matches on
  folder path with no extension filter, and definitions live anywhere in this fork, so a
  folder mapped via `filesPaths` typed the definitions sitting in it — each then showing
  up in its own class's table, invalid on every required field. `resolve()` now excludes
  definitions outright, which is the fork's equivalent of upstream refusing to bind its
  class folder.
- **A base generated by upstream is repaired rather than mistaken for hand-edited.**
